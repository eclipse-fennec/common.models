# Contributing to Eclipse Fennec — Common Models
...
* This repository: https://github.com/eclipse-fennec/common.models
* Issue tracker: https://github.com/eclipse-fennec/common.models/issues
...
* Search the [issue tracker](https://github.com/eclipse-fennec/common.models/issues)
  first — your problem may already be reported.
* When filing a new issue, identify which model module is affected
  (e.g. `net.opengis.gml3.model`), the Fennec version, and a minimal
  reproducer.
...
7. **Wait for CI.** All status checks (build matrix on Linux + Windows,
   license header check) must be green before review. See
...
```bash
git commit -s -m "Add WMS 1.3 schema generation"
```
...
Generated EMF sources carry the header configured via the `copyrightText`
property of each module's `.genmodel`. The header pattern and excluded
paths are defined in [`.licenserc.yaml`](.licenserc.yaml). The check is
run locally with:
...
The README has a complete [Adding a New Model Module](readme.md#adding-a-new-model-module)
checklist — directory layout, `bnd.bnd` template, genmodel settings,
versioning rules, and the steps for registering the module in the BOM /
BND library. Follow it before opening a PR that introduces a new model.

## Coding style

* **Generated EMF code:** do not hand-edit. Always regenerate from the
  `.genmodel` and commit the result.
* **Bundle versions:** follow the underlying specification (e.g. WFS 2.0
  → `2.0.0.x`). See the README for the rule.
* **Java 21** is the compliance level for every module.

## Build prerequisites

* Java 21. CI runs on Ubuntu and Windows.
...
* Issues: https://github.com/eclipse-fennec/common.models/issues
