// Sync the curated, user-facing pages from ../docs into ./docs/guides for VitePress.
//
// Single source of truth stays <repo>/docs/. Publication is an explicit ALLOWLIST
// (see guides.mjs) — internal pages (e.g. the CI pipeline description) are
// deliberately NOT published. Cross-links inside published pages that point at a
...
// Branch/ref used for the GitHub blob fallback links (unpublished docs are browsed
// on GitHub, not published). Passed by CI; defaults to main for local builds.
const branch = process.env.DOCS_BRANCH || 'main';
const blobBase = `https://github.com/eclipse-fennec/common.models/blob/${branch}/docs`;

const sections = [{ items: GUIDES, dir: 'guides' }];
